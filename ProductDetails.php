              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
                <div class="x_panel">
                  <div class="x_title text-center">
					...
					$requestedPROD = $_GET["PRODID"];
					
					
					$qproddetails = "SELECT PRODUCTNAME, PRODUCTID, SIZE FROM PRODUCTS WHERE PRODUCTID = '$requestedPROD'";
						
					$runproddetails = mysqli_query($dbc, $qproddetails);
					while($row = mysqli_fetch_array($runproddetails, MYSQLI_ASSOC)){
                  ...
                  <div class="x_content">
					...
					<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal form-label-left"  >
						...
						$qproddetails = "SELECT * FROM PRODUCTS WHERE PRODUCTID = '$requestedPROD'";
						
						$runproddetails = mysqli_query($dbc, $qproddetails);
						$fetchproddetails = mysqli_fetch_array($runproddetails, MYSQLI_ASSOC);
						
						$prodname = $fetchproddetails['PRODUCTNAME'];
						$prodsize = $fetchproddetails['SIZE'];
						
						
						$qgetmatused = "SELECT * FROM MATERIALSUSED WHERE PRODUCTID = '$requestedPROD'";
						$rungetmatused = mysqli_query($dbc, $qgetmatused);
						...
						while($fetchgetmatused = mysqli_fetch_array($rungetmatused, MYSQLI_ASSOC)){	
							$matcode = $fetchgetmatused['MATERIALCODE'];
							$qgetmatname = "SELECT MATERIALNAME FROM MATERIALS WHERE MATERIALCODE = '$matcode'";
							$runqgetmatname = mysqli_query($dbc, $qgetmatname);
							$fetchmatname = mysqli_fetch_array($runqgetmatname, MYSQLI_ASSOC);
							$materialname = $fetchmatname['MATERIALNAME'];
							$quantity = $fetchgetmatused['QUANTITY'];
							$unit = $fetchgetmatused['UNIT'];
							$qconvert = "SELECT * FROM REF_CONVERSION WHERE CONVERTEDUNIT = '$unit'";
							$runconvert =mysqli_query($dbc, $qconvert);
							$fetchconvert = mysqli_fetch_array($runconvert, MYSQLI_ASSOC);
							$convertvalue = $fetchconvert['CONVERTEDVALUE'];
							$convertunit = $fetchconvert['UNIT'];
							$newvalue = $quantity / $convertvalue;
							
							array_push($matname, $materialname);
							array_push($matqty, $newvalue);
							array_push($matunit, $convertunit);
						}
						
						for($index = 0; $index < sizeof($matname); $index++){
							echo "<tr><td align='center'>".$matname[$index]."</td>";
							echo "<td align='center'>".$matqty[$index]."</td>";
							echo "<td align='center'>".$matunit[$index]."</td></tr>";
						}
