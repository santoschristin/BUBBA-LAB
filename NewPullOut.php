...
<html lang="en">
	...
	if(isset($_POST['submit'])){
		...
		$_SESSION['hasNegative'] = 0;
		$hasSelected = 0;
		
		if($_POST['dbranch'] == "Branches"){
			$_SESSION['message'] .= "You must select a branch!\\n";
		}

		else{
			#get branchcode
			$dbranch = $_POST['dbranch'];
			$dbranchquery = "SELECT BRANCHCODE FROM REF_BRANCHES WHERE BRANCHNAME = '{$dbranch}'";
			$dbranchresult = mysqli_query($dbc, $dbranchquery);
			$dbranchrow=mysqli_fetch_array($dbranchresult,MYSQLI_ASSOC);
			$_SESSION['dbranchcode'] = $dbranchrow['BRANCHCODE'];
			...
			while($materialcoderow = mysqli_fetch_array($getmaterialresult,MYSQLI_ASSOC)){
				if($_POST['quantity'.$materialcoderow['MATERIALCODE']] != 0){
					$hasSelected = 1;
				}
				if(($materialcoderow['QUANTITYINSTOCK'] - $_POST['quantity'.$materialcoderow['MATERIALCODE']]) >= 0 && $hasNegative == 0){
					$_SESSION['quantity'.$materialcoderow['MATERIALCODE']] = $_POST['quantity'.$materialcoderow['MATERIALCODE']];
				}
				else{
					$_SESSION['hasNegative'] = 1;
				}	
			}
		}
		
		if($_SESSION['hasNegative'] == 1){
			$_SESSION['message'] .= "Pullout quantity cannot exceed quantity in stock!\\n";
		}
		
		if($hasSelected == 0){
			$_SESSION['message'] .= "No new changes!\\n";
		}
				
		header("Location: NewPullout2.php");
	}
                ...
                <div class="x_panel">
				 ...
				 <div class="x_title">
				  ...
				  <div class="form-group">
						 ...
						 <div class="form-group">
                          <label class="control-label col-md-4 col-sm-3 col-xs-12">Destination Branch: 
                        </label>
						  ...
						  <?php
								...
								$query2="select branchname from ref_branches"; 
								$result=mysqli_query($dbc,$query2);
								
								echo '<select name="dbranch" class="form-control col-md-7 col-xs-12">';
								echo '<option default>Branches</option>';
								while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))

								{
									echo '<option value="'.$row['branchname'].'" class="form-control col-md-7 col-xs-12"> '.$row['branchname'].' </option>';
								}
                        ...
                        <div class="col-md-6 col-sm-6 col-xs-12">
                          <?php
							...
							$materialquery="SELECT MATERIALCODE, MATERIALNAME, UNIT, QUANTITYINSTOCK
									  FROM MATERIALS";
							$materialresult = mysqli_query($dbc, $materialquery);
							while ($row=mysqli_fetch_array($materialresult,MYSQLI_ASSOC)){
								...
								echo"<td ><input align='center' class='form-control col-md-6 col-xs-12' value = '0' type='number' name='quantity".$row['MATERIALCODE']."'/></td></tr>";
