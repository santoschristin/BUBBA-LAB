              ...
              <div class="col-md-12 col-sm-12 col-xs-12">
				...
				<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" class="form-horizontal form-label-left"  >
						...
						<table id="datatable-responsive" class="table table-hover table-bordered dt-responsive nowrap" cellspacing="0" width="100%">
						 <thead>
						<tr>	
						<th width="20%"><div align="center"><b>Product Name
						</div></b></th>
						<th width="15%"><div align="center"><b>Size
						</div></b></th>
						<th width="10%"><div align="center"><b>Unit Price
						</div></b></th>
						<th width="15%"><div align="center"><b>Quantity
						</div></b></th>
						<th width="15%"><div align="center"><b>Amount
						</div></b></th>
						...
						while($fetchproductsordered = mysqli_fetch_array($runquery, MYSQLI_ASSOC)){
							...
							echo "<tr><td  align='center'>".$prodname."</td>";
							echo "<td align='center'>".$size."</td>";
							echo "<td align='center'>".$unitprice."</td>";
							echo "<td align='center'>".$qty."</td>";
							echo "<td align='center'>".$amount."</td></tr>";
						}
					...
					$updatesales = "UPDATE `bubbalab`.`dailysales` SET `TOTALSALES`='$dailysales' WHERE `DATE`= CURDATE()";
					...
					while($fetchproductsordered = mysqli_fetch_array($runquery, MYSQLI_ASSOC)){
						...
						if($prodtype == 1){
							...
							$qgettopqty = "SELECT QUANTITYINSTOCK FROM MATERIALS WHERE MATERIALCODE = '$topmat'";
							...
							$updatetopqty = "UPDATE `bubbalab`.`materials` SET `QUANTITYINSTOCK`='$topqty' WHERE `MATERIALCODE`='$topmat'";
							...
							$qgetsugarqty = "SELECT QUANTITYINSTOCK FROM MATERIALS WHERE MATERIALCODE = '$sugarmat'";
							...
							$updatesugarqty = "UPDATE `bubbalab`.`materials` SET `QUANTITYINSTOCK`='$sugarqty' WHERE `MATERIALCODE`='$sugarmat'";
						...
						while($fetchingre = mysqli_fetch_array($runingre, MYSQLI_ASSOC)){
								...
								$qgetmatqty = "SELECT QUANTITY FROM MATERIALSUSED WHERE PRODUCTID = '$prodid' AND MATERIALCODE = '$matcode'";
								...
								if($matcode == 10){
									...
									$sugarused = $fetchmatqty['QUANTITY'];
									$matqty = ($sugarused * $sugarvalue) * $qty;
								}
								else{
									$matqty = $fetchmatqty['QUANTITY'] * $qty;
								}
								$qgetinvvalue = "SELECT QUANTITYINSTOCK FROM MATERIALS M WHERE MATERIALCODE = $matcode";
								...
								$updateqtyinstock = "UPDATE `bubbalab`.`materials` SET `QUANTITYINSTOCK`='$invvalue' WHERE MATERIALCODE = '$matcode'";
								$runupdatestock = mysqli_query($dbc, $updateqtyinstock);
							}
						}
								$cash = $_POST['cashgiven'];
								$change = $cash - $total;
					   ...
					   <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Total Orders:
                        </label>
					   ...
					   <div class="form-group">
                        <label class="control-label col-md-3 col-sm-3 col-xs-12">Cash Given:
                        </label>
                        <div class="col-md-6 col-sm-6 col-xs-12">
                        
						  <p><input type="text"  size="20" maxlength="30" class="form-control col-md-7 col-xs-12"name="cashgiven"/>
                        </div>
                      </div>
					   ...
					   <div class="form-group">
                        <div class="col-md-6 col-sm-6 col-xs-12 col-md-offset-5">
						  ...
						  <button type="submit" name="nexttrans" class="btn btn-primary">Next Transaction</button>
                        </div>
                      </div>
...
<script src = "jquery.js"></script>
	...
	$(document).ready(function()
	{
		$("#ad").click(function() {
			...
			var getmaterialname = "<?php $materialname = "SELECT MATERIALCODE, MATERIALNAME FROM MATERIALS "; $result=mysqli_query($dbc,$materialname); echo "<option default>Materials</option>"; while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))  {  echo "<option value = '{$row['MATERIALCODE']}'>{$row['MATERIALNAME']}</option>";} ?>";
			var getunit = "<?php $unit = "SELECT UNIT FROM REF_UNITS"; $result = mysqli_query($dbc, $unit);  echo "<option default>Units</option>"; while($row=mysqli_fetch_array($result,MYSQLI_ASSOC)) { echo "<option value = '{$row['UNIT']}'>{$row['UNIT']}</option>";}?>";
			
			$("#tr1").append('<tr>');
			$("#mat").append('<select name = "materials[]" class="form-control col-md-7 col-xs-12"  placeholder="Materials" > '  + getmaterialname + ' </select>' );
