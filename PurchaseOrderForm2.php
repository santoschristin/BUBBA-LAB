<!DOCTYPE html>
	...
	if (isset($_POST['submit'])){
		...
		$query3 = "SELECT SUPPLIERID, EMAILADDRESS, SUPPLIERNAME, ADDRESS, 2NDADDRESS, CONTACTPERSON FROM REF_SUPPLIERS WHERE SUPPLIERID = '$supplier' ";
		$result3 = mysqli_query($dbc, $query3);
		while($row = mysqli_fetch_array($result3, MYSQLI_ASSOC)){
			$supplierEmail = $row['EMAILADDRESS'];
			$supplierName = $row['SUPPLIERNAME'];
			$address = $row['ADDRESS'];
			$secondaddress = $row['2NDADDRESS'];
		...
		$bodyContent   = '<p> '.$supplierName.' </p>';
		$bodyContent   .= '<p> '.$contactPerson.' </p>';
		
		$bodyContent   .= '<p> '.$address.' <br> '.$secondaddress.' </br></p>';
		
		$bodyContent   .= '<p> </p>';
		$bodyContent   .= '<p> </p>';
		$bodyContent   .= '<p> </p>';
	
		$bodyContent   .= '<p> Good day, '.$contactPerson.' </p>';
		
		$bodyContent   .= '<p> We would like to purchase the following: </p>';
		
		$bodyContent   .= '<p> </p>';
		$bodyContent   .= '<p> </p>';
		
		$bodyContent   .= '<table  class="table table-hover table-bordered" cellspacing="0" width="100%">
		
		<thead>
		<tr>	
		<th width="20%"><div align="center"><b>Item
		</div></b></th>
		<th width="15%"><div align="center"><b>Quantity Ordered
		</div></b></th>
		<th width="20%"><div align="center"><b>Unit
		</div></b></th>
		...
		<td width="20%"><div align="center">'.$matName.'
		</div></td>
		<td width="15%"><div align="center">'.$qtyO.'
		</div></td>
		<td width="20%"><div align="center">'.$unit.'
		</div></td>
		...
		$bodyContent   .= '<p> Please confirm that you received this order by clicking the <b><a href="http://localhost/APPDEV/production/ConfirmOrder.php?PO='.$po.'" >link. </a> </b> </p>';
		
		$bodyContent   .= '<p> </p>';
		
		$bodyContent   .= '<p>If you should have any problems or queries about this order please do not hesitate to contact us.</p>';
		
		$bodyContent   .= '<p> </p>';
		$bodyContent   .= '<p>Contact Details: 
							<br> Cellphone number: 09499905431 </br>
							<br> Telephone number: 123-4567  </br>
							<br> Email address: bubbalab.marikina@gmail.com </br></p>';
		
		$bodyContent   .= '<p> All the best, 
							<br> Juana Dela Cruz </br>
							<br> Operations Supervisor</br></p>';
...
self.location='PurchaseOrderForm2.php?SUPID=' + val ;
}
